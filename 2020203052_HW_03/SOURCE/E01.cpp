...
void mouseButton(GLFWwindow* window, int button, int action, int mods){
    ...
    if(action==GLFW_PRESS&&button==GLFW_MOUSE_BUTTON_LEFT){
        ...
        inputMode=InputMode::MAKING;

        point[pointnum][0]=xw; point[pointnum][1]=yw;
        pointnum++;
    }

    if(action==GLFW_RELEASE&&pointnum!=3){
        inputMode = InputMode::MAKING;
...
void mouseMove(GLFWwindow* window, double x, double y){
    if(inputMode==InputMode::MAKING){
        screen2world((float)x,(float)y,point[pointnum][0],point[pointnum][1]);
    }
}