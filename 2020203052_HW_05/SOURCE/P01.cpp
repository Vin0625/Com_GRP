...
void drawSphere(float radius,const vec3& color){
    ...
    gluSphere(sphere,radius*0.5,36,36);
}

void drawCylinder(float radius,float height,const vec3& color){
    setDiffuseColor(color);

    gluCylinder(cylinder,radius*0.5,radius*0.5,height,72,5);
}

void drawDisk(float inner_radius, float outer_radius,const vec3& color){
    setDiffuseColor(color);

    gluDisk(disk,inner_radius*0.5,outer_radius*0.5,72,5);
}
...
void render(GLFWwindow* window){
    ...
    {
        case 1:
            drawSphere(1.0,vec3(0.95,0.95,0.95));
            break;
        case 2:
            drawCylinder(1.0,1.0,vec3(0.95,0.95,0.95)); 
            break;
        case 3:
            drawDisk(0.5,1.0,vec3(0.95,0.95,0.95));
            break;
        case 4:
