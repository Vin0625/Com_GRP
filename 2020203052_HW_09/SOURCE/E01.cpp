...
void init(){
    ...
    gluQuadricDrawStyle(cylinder,GLU_OUTSIDE);
...
void setupLight(const vec4& p, int i){
    ...
    if(i==2){
        ...
        glLightf(GL_LIGHT0+1,GL_SPOT_CUTOFF,180);
    }
}
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods){
    if(action==GLFW_PRESS||action==GLFW_REPEAT){
        ...
        case GLFW_KEY_UP:
            ...
            break;
        case GLFW_KEY_DOWN:
            period=(float)std::max(period-0.1,0.1);
            break;
        case GLFW_KEY_V:
            view = (view+1)%3;
            break;
