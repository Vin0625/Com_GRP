...
inline vec3 ambient(const Light& l,int iobject){
    vec3 I(0,0,0);

    for(int i=0; i<3;i++){
        I[i]+=m_ambient[iobject][i]*l.ambient[i];
    }

    return I;
}

vec3 phong(const vec3& n, const vec3& v, const Light& l,const vec3& r,int iobject){
        vec3 I = ambient(l,iobject);
        ...
        if(lambertian>0){
            ...
            for(int i=0; i<3;i++){
            I[i]+=m_diffuse[iobject][i]*lambertian*l.diffuse[i];
            I[i]+=m_specular[iobject][i]*specular*l.specular[i];
        }
        }
...
vec3 intensity(const Ray&ray, const Light l[],int nights, int depth, int E=-1){
    vec3 I(0,0,0);
    ...
    if(iObject !=-1){
        vec3 objectColor = sphereColors[iObject];
        int index=iObject;
        if(!colorch){
            index=7;
            objectColor=sphereColors[index];
        }
        for(int i=0;i<nLights;i++){
            ...
            if(jObject==-1){
                ...
                I+=objectColor *phong(n,v,l[i],r,index);
            }else{
                I+=ambient(l[i],index);
            }
        }
...
void init(){
    ...
    sphereColors[0] = vec3(1.0f, 0.0f, 0.0f); 
    sphereColors[1] = vec3(0.0f, 1.0f, 0.0f);
    sphereColors[2] = vec3(0.0f, 0.0f, 1.0f); 
    sphereColors[3] = vec3(1.0f, 1.0f, 0.0f); 
    sphereColors[4] = vec3(1.0f, 0.5f, 0.0f); 
    sphereColors[5] = vec3(0.5f, 0.0f, 0.5f); 
    sphereColors[6] = vec3(0.0f, 1.0f, 1.0f);
    sphereColors[7] = vec3(0.5f, 0.5f, 0.5f); 

    for (int i = 0; i < 7; i++) {
        m_ambient[i] = sphereColors[i] * 0.1f; 
        m_diffuse[i] = sphereColors[i];          
        m_specular[i] = vec3(1.0f, 1.0f, 1.0f) * 0.5f; 
    }

    m_ambient[7]={0.1,0.1,0.1};
    m_diffuse[7]={0.95,0.95,0.95};
    m_specular[7]={0.5,0.5,0.5};
    ...
    float d=1.0f;
    float r=1.414f*d;
    center_world[nSpheres]=vec3(0,0,0);
    radius[nSpheres]=0.7f;
    nSpheres++;

    center_world[nSpheres]=vec3(-d,-d,0);
    radius[nSpheres]=0.5f;
    nSpheres++;
    ...
    center_world[nSpheres]=vec3(0,0,r);
    radius[nSpheres]=0.5f;
    nSpheres++;
    ...
    cout<<"keyboard input: [1:9] for tracing depth"<<endl;
}
...
void increaseSpecular(){
    for(int j=0;j<8;j++){
        for(int i=0;i<3;i++){
            int o = colorch ? j : nSpheres;
        m_specular[j][i]=std::min(m_specular[j][i]+0.1f,1.0f);
    }
    }
...
void decreasingSpecular(){
    for(int j=0;j<8;j++){
        for(int i=0;i<3;i++){
        int o = colorch ? j : nSpheres;
        m_specular[j][i]=std::max(m_specular[j][i]-0.1f,0.0f);
    }
    }
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods){
    if(action == GLFW_PRESS|| action == GLFW_REPEAT){
        ...
        case GLFW_KEY_DOWN:
            ...
            break;
        case GLFW_KEY_1:
            setRayTracingDepth(1);
            break;
        case GLFW_KEY_2:
            setRayTracingDepth(2);
            break;
        case GLFW_KEY_3:
            setRayTracingDepth(3);
            break;
        case GLFW_KEY_4:
            setRayTracingDepth(4);
            break;
        case GLFW_KEY_5:
            setRayTracingDepth(5);
            break;
